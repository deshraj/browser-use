...
## Run with memory (personalized agent)

You can configure the agent to use user memories, providing a personalized and reliable experience.
...
messages = [
    {"role": "user", "content": "Hey, I'm Mark. I love exploring new places, especially in the mountains. I prefer staying in airbnb accommodations."},
...
agent = Agent(
    task="Planning to go somewhere near San Francisco. Plan a trip for me.",  # Task for the agent
...
You can find an example of a personal travel planner in the [examples/use-cases/personal_travel_planner.py](https://github.com/browser-use/browser-use/blob/main/examples/use-cases/personal_travel_planner.py) file.

<Note>
  The `user_id` parameter is optional. If not specified, the agent will not use memories of a user.
</Note>
