...
You can customize the memory system with specific providers for embeddings and vector storage:
...
agent = Agent(
    ...
    memory_config={
        ...
        "vector_store": {"provider": "faiss", "config": {"path": "./chroma_db"}},
    },
)
```

Alternatively, you can use Pydantic BaseModel classes for more type-safe configuration:
...
class EmbedderConfig(BaseModel):
    provider: str = "openai"
    config: Dict[str, Any] = {"model": "text-embedding-3-large"}

class VectorStoreConfig(BaseModel):
    provider: str = "faiss"
    config: Dict[str, Any] = {"path": "./chroma_db"}
...
# Initialize with Pydantic models
agent = Agent(
    task=task_description,
    llm=llm,
    memory_config=MemoryConfig(llm=llm).model_dump()
)
```

<Note>
  For providers other than OpenAI, Gemini, and Ollama, Browser Use automatically uses Hugging Face embedding model.
</Note>
